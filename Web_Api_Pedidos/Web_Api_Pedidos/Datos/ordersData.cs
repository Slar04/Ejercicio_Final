                ...
                using (var dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        oLista.Add(new Order()
                        {
                            ...
                            ShipVia = Convert.ToInt16(dr["ShipVia"]),
            ...
            using (var conexion = new SqlConnection(cd.GetCadenaSQL()))
            {
                ...
                using (var dr = cmd.ExecuteReader())
                {
                    while (dr.Read())
                    {
                        ...
                        oCategory.RequiredDate = Convert.ToDateTime(dr["RequiredDate"].ToString());
                        oCategory.ShippedDate = Convert.ToDateTime(dr["ShippedDate"].ToString());
                        oCategory.ShipVia = Convert.ToInt16(dr["ShipVia"].ToString());
                        oCategory.Freight = Convert.ToDecimal(dr["Freight"].ToString());
